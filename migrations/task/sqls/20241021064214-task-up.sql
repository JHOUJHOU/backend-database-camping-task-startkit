...
VALUES -- USER 表 id, CREDIT_PACKAGE 表 id, 組合包堂數, 組合包價格
	(
...
-- 3-2. 新增：承1，為三名教練新增專長資料至 `COACH_LINK_SKILL` ，資料需求如下：
    ...
    FROM 
        "COACH"
    CROSS JOIN 
    (SELECT id FROM "SKILL" WHERE name = '重訓') "SKILL";
...
SET 
	...
	status = '上課中'
WHERE status = '即將授課' AND user_id = (SELECT id FROM "USER" WHERE name = '王小明'); 
...
FROM "COURSE_BOOKING"
WHERE 
	"COURSE_BOOKING".status NOT IN ('即將授課','課程已取消') -- NOT IN 是找你()中沒有的狀態，所以會找出上課中
	AND
	user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io')
GROUP BY user_id;
...
WHERE "SKILL"."name" = '重訓'
ORDER BY "COACH"."experience_years" DESC; -- DESC 遞減(由大到小)
...
INNER JOIN "SKILL" ON "COACH_LINK_SKILL"."skill_id" = "SKILL"."id"
GROUP BY "SKILL"."name"
ORDER BY coach_total DESC -- DESC 遞減(由大到小)
LIMIT 1; -- 從第一筆顯示
...
SELECT
    COUNT(distinct(status)) AS "預約會員人數"
FROM "COURSE_BOOKING"
WHERE
